...
import 'model_generator.dart' as origin;
import 'sintaxe_extend.dart';

class ModelGenerator extends origin.ModelGenerator {
  ...
  ModelGenerator(this._rootClassName,
      ...
      List<origin.Hint>? hints,
      this._useStorage = false])
      : super('') {
    if (hints != null) {
      this.hints = hints;
    } else {
      this.hints = <origin.Hint>[];
    }
  }

  origin.Hint? _hintForPath(String path) {
  ...
  /// might be returned
  @override
  origin.DartCode generateUnsafeDart(String rawJson) {
    ...
    final code =
        _importStorageClass(allClasses.map((c) => c.toString()).join('\n'));
    return origin.DartCode(code, warnings);
  }
